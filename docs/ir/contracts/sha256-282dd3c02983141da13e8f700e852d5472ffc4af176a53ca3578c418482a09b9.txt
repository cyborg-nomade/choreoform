...
the exact bytes of this document. The checker recognizes its serialization but
does not implement its meaning. No tool may plan or execute these examples just
because their JSON Schema and digest checks pass.
...
source digests, selected linking rules, canonicalization invariants, malformed
JSON, and negative mutations. Its `--revisions` option prints recalculated
fixture digests for review; it never rewrites fixtures. It does not implement
